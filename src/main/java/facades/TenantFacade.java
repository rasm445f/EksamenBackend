//package facades;
//
//
//import dtos.RentalDto;
//import dtos.TenantDto;
//import entities.House;
//import entities.Rental;
//import entities.Tenant;
//import errorhandling.API_Exception;
//import javassist.NotFoundException;
//
//import javax.persistence.EntityManager;
//import javax.persistence.EntityManagerFactory;
//import javax.persistence.TypedQuery;
//import java.util.ArrayList;
//import java.util.List;
//
///**
// * @author lam@cphbusiness.dk
// */
//public class TenantFacade {
//    private static EntityManagerFactory emf;
//    private static TenantFacade instance;
//
//    private TenantFacade() {
//    }
//
//    public static TenantFacade getTenantFacade(EntityManagerFactory _emf) {
//        if (instance == null) {
//            emf = _emf;
//            instance = new TenantFacade();
//        }
//        return instance;
//    }
//
//    private EntityManager getEntityManager() {
//        return emf.createEntityManager();
//    }
//
//    public Tenant createTenant(Tenant tenant) {
//        EntityManager em = getEntityManager();
//        try {
//            em.getTransaction().begin();
//            em.persist(tenant);
//            em.getTransaction().commit();
//        } finally {
//            em.close();
//        }
//        return tenant;
//    }
//
////    public Tenant createTenant(Tenant tenant, Harbor harbor) {
////        EntityManager em = getEntityManager();
////
////        if(harbor==null){
////            Harbor defaultHarbor = new Harbor();
////            tenant.addRole(defaultRole);
////        }
////        else{
////            tenant.addRole(role);
////        }
////        try {
////            em.getTransaction().begin();
////            em.persist(tenant);
////            em.getTransaction().commit();
////        } finally {
////            em.close();
////        }
////        return tenant;
////    }
//
//
//    public List<TenantDto> getAllTenants() throws NotFoundException {
//        EntityManager em = getEntityManager();
//        try {
//            TypedQuery<Tenant> query = em.createQuery("SELECT u FROM Tenant u", Tenant.class);
//            if (query == null) {
//                throw new NotFoundException("Can't find any tenants");
//            }
//            List<Tenant> tenants = query.getResultList();
//            return TenantDto.getTenantDtos(tenants);
//        } finally {
//            em.close();
//        }
//    }
//
////    public List<TenantDto> getAllTenantsOfBoatID(int boatId) throws NotFoundException {
////        EntityManager em = getEntityManager();
////
////        try {
////            TypedQuery<Tenant> query = em.createQuery("SELECT DISTINCT ob.tenantID FROM TenantRental ob WHERE ob.id.boatID = :boatId", Tenant.class);
////            query.setParameter("boatId", boatId);
////            if (query == null) {
////                throw new NotFoundException("Can't find any tenants");
////            }
////            List<Tenant> tenants = query.getResultList();
////            return TenantDto.getTenantDtos(tenants);
////        } finally {
////            em.close();
////        }
////    }
//
//    public TenantDto getTenantById(int id) throws API_Exception {
//        EntityManager em = getEntityManager();
//
//        Tenant tenant = em.find(Tenant.class, id);
//        if (tenant == null)
//            throw new API_Exception("There's no tenant with that id", 404);
//        em.close();
//        return new TenantDto(tenant);
//
//
//    }
//
//    public TenantDto deleteTenant(int id) throws API_Exception {
//        EntityManager em = getEntityManager();
//        Tenant tenant;
//        try {
//            tenant = em.find(Tenant.class, id);
//            if (tenant == null) {
//                throw new API_Exception("Can't find a tenant with the tenantname: " + id);
//            }
//            em.getTransaction().begin();
//            em.remove(tenant);
//            em.getTransaction().commit();
//            return new TenantDto(tenant);
//        } finally {
//            em.close();
//        }
//    }
//
//
//    public TenantDto updateTenant(int id, Tenant tenantUpdate) {
//        EntityManager em = getEntityManager();
//        Tenant tenant;
//        try {
//            tenant = em.find(Tenant.class, id);
//            em.getTransaction().begin();
//            tenant.setTenantName(tenantUpdate.getTenantName());
//            tenant.setTenantJob(tenantUpdate.getTenantJob());
//            em.getTransaction().commit();
//            return new TenantDto(tenant);
//        } finally {
//            em.close();
//        }
//
//    }
//
//    public List<TenantDto> getAllTenantsOfHouseID(int houseId) throws NotFoundException {
//        EntityManager em = getEntityManager();
//
//        try {
//            TypedQuery<Tenant> query = em.createQuery("SELECT t FROM Tenant t JOIN t.rentals r JOIN r.houseID h WHERE h.id = :houseId", Tenant.class);
//            query.setParameter("houseId", houseId);
//            if (query == null) {
//                throw new NotFoundException("Can't find any rentals");
//            }
//            List<Tenant> tenants = query.getResultList();
//            return TenantDto.getTenantDtos(tenants);
//        } finally {
//            em.close();
//        }
//    }
//
//
//
////    public OwnerDto addOwner(Boat boat) throws API_Exception {
////        EntityManager em = getEntityManager();
////        try {
////            em.getTransaction().begin();
////            em.persist(boat);
////            em.getTransaction().commit();
////        } finally {
////            em.close();
////        }
////        BoatDto theFan = new BoatDto(boat);
////        return theFan;
////    }
////    public List<BoatDto> getAllBoatsFromID(int id){
////        EntityManager em = getEntityManager();
////        try {
////            TypedQuery<Boat> query = em.createQuery("SELECT f FROM Boat f WHERE f.userid = ?1", Boat.class)
////                    .setParameter(1,id);
////            List<Boat> favoritesList = query.getResultList();
////            return BoatDto.getFavoriteDTOs(favoritesList);
////        } finally {
////            em.close();
////        }
////    }
//
//
//}
//
//
